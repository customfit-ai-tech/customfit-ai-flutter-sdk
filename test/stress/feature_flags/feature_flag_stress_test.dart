  ...
  group('Feature Flag Stress Tests', () {
    ...
    group('Rapid Sequential Evaluation Stress', () {
      ...
      test('should handle rapid sequential boolean flag evaluations', () async {
        ...
        for (int i = 0; i < 1000; i++) {
          final result = client.getValue<bool>('rapid_bool_flag_$i', false);
      ...
      test('should handle rapid sequential string flag evaluations', () async {
        final stopwatch = Stopwatch()..start();
        for (int i = 0; i < 1000; i++) {
          final result = client.getValue<String>('rapid_string_flag_$i', 'default');
      ...
      test('should handle rapid sequential number flag evaluations', () async {
        final stopwatch = Stopwatch()..start();
        for (int i = 0; i < 1000; i++) {
          final result = client.getValue<double>('rapid_number_flag_$i', 0.0);
      ...
      test('should handle rapid sequential JSON flag evaluations', () async {
        ...
        for (int i = 0; i < 500; i++) {
          final result = client.getValue<Map<String, dynamic>>('rapid_json_flag_$i', defaultJson);
    ...
    group('Large Value Handling Stress', () {
      ...
      test('should handle very large string flag values', () async {
        ...
        for (int i = 0; i < 100; i++) {
          final result = client.getValue<String>('large_string_flag_$i', largeString);
          expect(result, isA<String>());
      ...
      test('should handle very large JSON flag values', () async {
        ...
        for (int i = 0; i < 50; i++) {
          final result = client.getValue<Map<String, dynamic>>('large_json_flag_$i', largeJson);
          expect(result, isA<Map<String, dynamic>>());
      ...
      test('should handle extremely large number arrays', () async {
        ...
        for (int i = 0; i < 20; i++) {
          final result = client.getValue<Map<String, dynamic>>(
    ...
    group('Concurrent Flag Evaluation Stress', () {
      ...
      test('should handle concurrent boolean flag evaluations', () async {
        ...
        for (int i = 0; i < 200; i++) {
          futures.add(
            Future(() => client.getValue<bool>('concurrent_bool_$i', false)),
          );
        }
      ...
      test('should handle concurrent mixed flag type evaluations', () async {
        ...
        for (int i = 0; i < 100; i++) {
          futures.add(
            Future(() => client.getValue<bool>('concurrent_mixed_bool_$i', false)),
          );
          futures.add(
            Future(
              () => client.getValue<String>('concurrent_mixed_string_$i', 'default'),
            ),
          );
          futures.add(
            Future(() => client.getValue<double>('concurrent_mixed_number_$i', 0.0)),
          );
        }
        final results = await Future.wait(futures);
      ...
      test(
        ...
        () async {
          ...
          for (int i = 0; i < 150; i++) {
            ...
            futures.add(
              Future(() async {
                await client.user.setUser(users[userIndex]);
                return client.getValue<bool>('user_specific_flag_$i', false);
              }),
            );
          }
          final results = await Future.wait(futures);
    ...
    group('Cache Performance Stress Tests', () {
      ...
      test('should handle cache pressure from many unique flags', () async {
        ...
        for (int i = 0; i < 2000; i++) {
          final result = client.getValue<bool>('unique_cache_flag_$i', false);
          expect(result, isA<bool>());
        }
        ...
        () async {
          ...
          for (int round = 0; round < 20; round++) {
            for (final flagName in flagNames) {
              final result = client.getValue<bool>(flagName, false);
              expect(result, isA<bool>());
            }
          }
          expect(true, isTrue);
      ...
      test(
        ...
        () async {
          ...
          for (int i = 0; i < 100; i++) {
            // Evaluate some flags
            for (int j = 0; j < 10; j++) {
              client.getValue<bool>('invalidation_flag_${i}_$j', false);
            }
    ...
    group('Error Handling Stress Tests', () {
      ...
      test('should handle rapid evaluations with invalid flag names', () async {
        ...
        for (int i = 0; i < 200; i++) {
          ...
          final result = client.getValue<bool>('${invalidName}_$i', false);
      ...
      test('should handle stress with malformed default values', () async {
        ...
        for (int i = 0; i < 100; i++) {
          ...
          client.getValue<String>('malformed_string_$i', '');
          client.getValue<double>('malformed_number_$i', double.nan);
          client.getValue<double>('malformed_number_infinity_$i', double.infinity);
          client.getValue<Map<String, dynamic>>('malformed_json_$i', <String, dynamic>{});
        }
        expect(true, isTrue);
      });
    });
    group('Memory Management Stress Tests', () {
      ...
      test(
        ...
        () async {
          ...
          for (int i = 0; i < 5000; i++) {
            client.getValue<bool>('memory_pressure_$i', false);
            client.getValue<String>('memory_pressure_string_$i', 'default');
            ...
            if (i % 100 == 0) {
              ...
              client.getValue<Map<String, dynamic>>('memory_pressure_json_$i', complexDefault);
            }
          }
      ...
      test('should handle garbage collection pressure', () async {
        ...
        for (int i = 0; i < 1000; i++) {
          ...
          client.getValue<Map<String, dynamic>>('gc_pressure_$i', tempMap);
