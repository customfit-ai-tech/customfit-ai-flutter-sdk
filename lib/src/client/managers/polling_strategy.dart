...
class PollingStrategy {
  ...
  PollingStrategy({
    ...
    int lowBatteryThreshold =
        15, // CFConstants.pollingStrategy.lowBatteryThreshold
    double lowBatteryMultiplier =
        2.0, // CFConstants.pollingStrategy.lowBatteryMultiplier
    int maxConsecutiveErrors =
        5, // CFConstants.pollingStrategy.maxConsecutiveErrors
  ...
  void _scheduleNextPoll() {
    synchronized(_lock, () {
      ...
      _timer = ManagedTimer(
        owner: 'PollingStrategy',
        duration: Duration(milliseconds: _currentIntervalMs),
        callback: () async {
          if (_isShutdown) return;
  ...
  ManagedTimer? _batteryMonitorTimer;
  ...
  void _monitorBatteryStatus() {
    ...
    _batteryMonitorTimer = ManagedTimer.periodic(
      owner: 'PollingStrategy_battery',
      duration: Duration(
          minutes: CFConstants.pollingStrategy.batteryCheckIntervalMinutes),
      callback: (_) async {
        if (_isShutdown) return;
    ...
    _batteryStateSubscription = _battery.onBatteryStateChanged.listen((state) {
      if (!_isShutdown && state == BatteryState.discharging) {
    ...
    synchronized(_lock, () {
      if (isLowBattery) {
        ...
        if (_currentIntervalMs < newInterval) {
          ...
          // Reschedule with new interval
          if (!_isPaused && !_isShutdown && _timer != null) {
            _timer?.dispose();
            _scheduleNextPoll();
          }
        }
