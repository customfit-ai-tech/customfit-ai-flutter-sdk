...
class ResourceRegistry {
  ...
  void register(DisposableResource resource) {
    if (_isShuttingDown) {
      Logger.w(
          'Cannot register resource during shutdown: ${resource.resourceId}');
      return;
    }
  ...
  Future<void> disposeAll() async {
    ...
    for (final resource in resources) {
      if (!resource.isDisposed) {
        futures.add(resource.dispose().catchError((error) {
          Logger.e(
              'Error disposing ${resource.resourceType} ${resource.resourceId}: $error');
        }));
      }
    }
