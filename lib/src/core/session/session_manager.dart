// lib/src/core/session/session_manager.dart
//
// Session management with time-based rotation strategy.
// Handles session lifecycle, rotation triggers, and persistence to ensure
// proper session tracking for analytics and feature flag evaluation.
//
...
import '../util/type_conversion_strategy.dart';
...
class SessionData {
  ...
  factory SessionData.fromMap(Map<String, dynamic> map) {
    return SessionData(
      ...
      createdAt:
          SafeTypeConverter.convertWithFallback<int>(map['createdAt'], 0),
      lastActiveAt:
          SafeTypeConverter.convertWithFallback<int>(map['lastActiveAt'], 0),
      appStartTime:
          SafeTypeConverter.convertWithFallback<int>(map['appStartTime'], 0),
      rotationReason: SafeTypeConverter.convertWithFallback<String?>(
          map['rotationReason'], null),
    );
  }
  ...
  factory SessionData.fromJson(String source) => SessionData.fromMap(
      SafeTypeConverter.convertWithFallback<Map<String, dynamic>>(
          json.decode(source), <String, dynamic>{}));
}
...
class SessionManager {
    ...
    if (_isInitializing) {
      ...
      if (_initCompleter != null) {
        try {
          ...
          return CFResult.error('SessionManager initialization failed: $e',
              exception: e);
        }
      }
    }
  ...
  Future<void> _initializeSession() async {
    ...
    await SimpleStorageHelper.init();
  ...
  Future<void> onAppForeground() async {
    ...
    final backgroundDuration =
        _lastBackgroundTime > 0 ? now - _lastBackgroundTime : 0;

    Logger.d(
        '🔄 App came to foreground after ${backgroundDuration}ms in background');
  ...
  Future<void> _storeCurrentSession() async {
    ...
    try {
      await SimpleStorageHelper.setString(
          _sessionStorageKey, session.toJson());
    } catch (e) {
  ...
  Future<void> _storeLastAppStartTime(int timestamp) async {
    try {
      await SimpleStorageHelper.setString(
          _lastAppStartKey, timestamp.toString());
    } catch (e) {
  ...
  Future<int> _getLastAppStartTime() async {
    try {
      final value = await SimpleStorageHelper.getString(_lastAppStartKey);
      if (value != null) {
  ...
  Future<void> _storeBackgroundTime(int timestamp) async {
    try {
      await SimpleStorageHelper.setString(
          _backgroundTimestampKey, timestamp.toString());
    } catch (e) {
