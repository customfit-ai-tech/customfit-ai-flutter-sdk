...
class WeakListenerManager<T extends Object> {
  ...
  List<T> getActiveListeners() {
    ...
    return _weakListeners.map((ref) => ref.target).whereType<T>().toList();
  }
  ...
  Future<void> notifyListenersAsync(
      Future<void> Function(T listener) callback) async {
    final activeListeners = getActiveListeners();
    Logger.d(
        '[$_name] Notifying ${activeListeners.length} listeners asynchronously');
