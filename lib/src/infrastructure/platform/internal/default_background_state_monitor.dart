...
import '../interfaces/background_state_monitor.dart';
import '../../../core/session/session_manager.dart';
// AppState and BatteryState now imported via background_state_monitor interface

// BackgroundStateMonitor interface now imported from interfaces/background_state_monitor.dart

/// Default implementation of background state monitoring.
class DefaultBackgroundStateMonitor
    with WidgetsBindingObserver
    implements IBackgroundStateMonitor {
  // App state
  ...
  void _updateBatteryState(battery_plus.BatteryState batteryState) async {
    ...
    try {
      ...
      final newState = _mapBatteryState(batteryState);
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    final newAppState = _mapAppLifecycleState(state);
  ...
  void _notifyAppStateListeners() {
    ...
    final listeners = List<void Function(AppState)>.from(_appStateListeners);
  ...
  void _notifyBatteryStateListeners() {
    ...
    // Create a copy of the listeners list to avoid concurrent modification
    final listeners =
        List<void Function(BatteryState)>.from(_batteryStateListeners);
  ...
  // Get current battery level (0-100) - legacy method
  int getCurrentBatteryLevel() => _currentBatteryLevel;

  // Get battery level as double (0.0 to 1.0) - interface method
  @override
  double getBatteryLevel() => _currentBatteryLevel / 100.0;

  // Start monitoring - interface method
  @override
  ...
  // Helper method to map battery_plus battery state to our enum
  BatteryState _mapBatteryState(battery_plus.BatteryState state) {
    switch (state) {
      case battery_plus.BatteryState.full:
        return BatteryState.full;
      case battery_plus.BatteryState.charging:
        return BatteryState.charging;
      case battery_plus.BatteryState.discharging:
        return BatteryState.unplugged;
      case battery_plus.BatteryState.unknown:
      default:
        return BatteryState.unknown;
    }
  }

  // Helper method to map Flutter's AppLifecycleState to our enum
  AppState _mapAppLifecycleState(AppLifecycleState state) {
    switch (state) {
      ...
      case AppLifecycleState.detached:
        return AppState.background; // Map detached to background
      default:
        return AppState.inactive;
    }
  }
...
// _BasicAppStateListener no longer needed - using function types directly
