// lib/src/network/config_fetcher_impl.dart
//
...
import '../../types.dart';
import '../../storage/internal/cache_manager_impl.dart';
...
class ConfigFetcherImpl implements IConfigFetcher {
  ...
  ConfigFetcherImpl(this._httpClient, this._config, this._userManager) {
    _fetchMutex.complete();
    _mutex.complete();
    Logger.d(
        'ConfigFetcher initialized with user: ${_userManager.getUser().userCustomerId}');
  }
  ...
  Future<bool> fetchConfig({String? lastModified, String? etag}) async {
    ...
    final cacheKey = 'cf_config_${_config.clientKey}_${user.userCustomerId}';
    ...
    // Check cache first
    final cachedConfig =
        await CacheManagerImpl.instance.get<Map<String, dynamic>>(cacheKey);
  ...
  Future<CFResult<bool>> _fetchConfigInternal(
    ...
    try {
      ...
      // Create strongly typed request with current user (gets latest properties)
      final currentUser = _userManager.getUser();
      ...
      await CacheManagerImpl.instance.put(
        cacheKey,
        configWithMetadata,
        policy: cachePolicy,
        metadata: {
          'clientKey': _config.clientKey,
          'userId': currentUser.userCustomerId ?? '',
        },
      );
  ...
  Future<CFResult<Map<String, String>>> _fetchMetadataInternal(
    ...
    try {
      ...
      final getResult =
          await _httpClient.fetchMetadata(targetUrl, headers: headers);
